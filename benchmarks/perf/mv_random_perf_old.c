...
/* Read 64 bit (RV64) insatruction counter */
...
static void mv_rvv_vs_scalar(int n, double sparsity)
{
    ...
    for (int it = 0; it < N_TESTS; ++it)
    {
        ...
        i_s_sum += diff64(i0, i1); // accumulate the number of instructions executed by the scalar function
    }
    ...
    for (int it = 0; it < N_TESTS; ++it)
    {
        ...
        i_v_sum += diff64(i0, i1); // accumulate the number of instructions executed by the vectorized function
    }
    ...
    double inst_s = (double)i_s_sum / N_TESTS;
    double inst_v = (double)i_v_sum / N_TESTS;

    double ipc_s = inst_s / c_s; /* Instructions-per-cycle scalar  */
    double ipc_v = inst_v / c_v; /* Instructions-per-cycle vector  */
    ...
    printf("n = %d, sparsity = %.4f, max_nnz_row = %d, "
           ...
           "cycles_serial = %.2f, cycles_vector = %.2f, speedup_cycles = %.2f, "
           "inst_serial = %.2f, inst_vector = %.2f, speedup_inst = %.2f, "
           "ipc_serial = %.3f, ipc_vector = %.3f, %s\n",
           n, sparsity, max_nnz,
           t_s, t_v, t_s / t_v,
           c_s, c_v, c_s / c_v,
           inst_s, inst_v, inst_s / inst_v,
           ipc_s, ipc_v,
           pass ? "PASS" : "FAIL");
