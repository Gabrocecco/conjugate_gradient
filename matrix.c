                     ...
                     int *csr_row_ptr)         // row pointer, where the first element of each row is stored
{
    ...
    for (int i = 1; i < upper_count; i++)
    {
        ...
        {
            // csr_row_ptr[row_ptr_index] = csr_row_ptr[row_ptr_index - 1] + count_element_in_row; // in the nxt position of csr_row_ptr we write the lat element of the previus entry plus the number of elements in the row
...
int main()
{
    ...
    upper_coo_to_csr(triag_dim, upper_count, values, row_index, col_index, csr_row_ptr);
